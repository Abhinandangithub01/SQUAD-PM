...
export default function TrelloKanban({ projectId }: TrelloKanbanProps) {
      ...
      <div className="flex gap-4 p-4 overflow-x-auto h-full">
        {STATUSES.map(status => (
          <div key={status.id} className="flex-shrink-0 w-80">
            <div className="bg-gray-100 rounded-lg p-3 h-full animate-pulse">
  ...
  return (
    <div className="flex gap-4 p-4 overflow-x-auto h-full bg-gradient-to-br from-blue-50 to-purple-50">
      {STATUSES.map((status) => (
        <TrelloList
          key={status.id}
          status={status}
...
function TrelloCard({ task, onClick, onDelete }: any) {
    ...
    <div className="relative group">
      ...
      {showMenu && (
        <>
          ...
          ></div>
          <div className="absolute top-8 right-2 w-48 bg-white rounded-lg shadow-xl border border-gray-200 py-1 z-20">
            <button
              onClick={(e) => {
                e.stopPropagation();
                onClick();
